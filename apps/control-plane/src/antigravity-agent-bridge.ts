...
function promptFor(
  ...
  if (settings.actionMode === "smart") {
    ...
    return [
      ...
      "玩家要求照料、播种或收割农田时可提交 life.crop-cycle 或 farm；本地 Agent 会先查询同世界已记录的家园主农田并扩大范围验证，识别不到才在房屋外建立并登记一块新农田。旧的远程农田/牧场默认只保留为次级据点，除非玩家明确要求使用旧设施。当前房屋本身绝不是农田，禁止把室内地面当作农田。",
      "自动建造必须使用已审查的 Skill/蓝图；本地服务会按完整蓝图边界把住宅放在房屋外 8–24 格、农田/牧场/树场等生产设施放在 16–40 格、刷怪机/刷石机等工业设施放在 40–64 格，并保护既有设施。不要自行扩大距离；只有玩家明确给出坐标时才保留该坐标。",
