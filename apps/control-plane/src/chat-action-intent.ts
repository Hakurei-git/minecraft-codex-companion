...
const STAY_COMMAND = /^\s*(?:(?:请|麻烦)\s*)?(?:你\s*)?(?:先\s*)?(?:在这(?:里)?待着|原地(?:等|等待|待命)|别跟了|停在这(?:里)?|stay)(?:\s*[,，]\s*(?:不要|别)(?:再)?(?:开始|执行|做)(?:任何|其他|别的)?任务)?\s*(?:吧|呀|啊|喵)?\s*[！!。.]?\s*$/iu;
...
const CONTINUE_BUILD_COMMAND = /(?:^|[，,。.!！?？\s])(?:(?:继续|接着|恢复)(?:(?:之前|当前|上次|原来|刚才)?(?:的)?(?:(?:住宅|房屋|房子|小屋|基地|农田|农场|仓库|围栏|塔|树场|刷怪塔|刷石机)(?:的)?)?)?(?:建造|搭建|施工|盖|造)|续建|接续(?:建造|搭建|施工)|(?:还没|没)(?:建|盖|造)完|(?:建造|搭建|施工)(?:任务)?(?:不动|没动|失败|卡住)|失败点(?:继续|恢复)?(?:建造|搭建|施工)?)(?:[，,。.!！?？\s]|$)/u;
