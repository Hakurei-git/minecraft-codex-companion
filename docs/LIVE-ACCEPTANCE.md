...
本清单用于直接源实例 `Dragons_ZH_1.20.1` 与 `1.21.1-NeoForge` 的最终实机验收；新版 EXE 不创建或启动 `-Codex` 副本。测试只访问 `127.0.0.1`，不读取或输出 API Key，不上传存档、目录、日志或截图。需要视觉证据时只截取 Minecraft 窗口。
...
- 可运行 `npm run smoke:live-food-survival-restart -- --apply --wait-seconds=300` 严格验证后台 `T` 消息“给我16个肉”。夹具在家园保护范围外建立可回滚场地，生成 18 头可猎杀成年牛以及各 1 头幼崽、命名牛和拴绳牛；后三者误伤任意一头即失败。验收必须观察 NPC 自己的真实攻击、击杀和生牛肉掉落，空熔炉的装料、燃料、点燃、熟牛肉分批出炉与累计取出，烹饪期间护主插队后恢复同一任务，以及原版保存退出、重进同一世界后沿用同一任务 ID。最终玩家必须实际取得 16 份熟肉，结果固定声明 `actor="ai-npc"`、`playerGameplayAssistanceUsed=false`、`usedMinecraftTChat=true`；聊天确认、任务成功文案或 fixture 直接给予物品均不能替代这些世界证据。脚本不检查 Minecraft 日志，结束时只清理专用标签实体、物品和精确记录方块，恢复玩家复活点及 NPC 完整状态，并默认正常关闭 Minecraft。隔离验收可通过 `--sandbox-profile-root=runtime/live-hmcl-profile` 沿用项目内 HMCL 测试配置，并把同一个 loopback `ControlBaseUri` 显式传入重启后的桥，防止误连主服务。
- 2026-08-15 食物生存与 Minecraft 重启严格实机结果（Forge JAR SHA-256 `4E177B4082D3614E575DCA8C833270975C959205B72041F5AB70E546402AB1D9`）：后台 `T` 精确发送“给我16个肉”，唯一 `provision-food,count=16,source=hunt,foodCategory=meat,destination=player` 任务由 AI NPC 自己完成。NPC 真实攻击 16 次、击杀 8 头可猎杀成年牛并取得 16 份生肉，幼崽、命名牛和拴绳牛三类受保护目标全部存活且违规为 0；空熔炉的输入、点燃、输出均被观察到，累计取出并物理交付 16 份熟肉。护主插队后恢复同一任务，原版保存退出与重进后继续相同任务 ID，最终 `status=succeeded`、`cleanup=food-survival:cleanup restored`。Minecraft 正常保存并关闭，未强制终止，也未使用物理键鼠、剪贴板或截图。脱敏证据见 [`LIVE-FOOD-SURVIVAL-RESTART-ACCEPTANCE-20260815.json`](LIVE-FOOD-SURVIVAL-RESTART-ACCEPTANCE-20260815.json)。
...
- 可运行 `node scripts/live-storage-smoke.mjs --apply --wait-seconds=180`，通过后台 `T` 聊天依次验证四条真实链路：从两个家园箱跨箱取出 8 个圆石并交付玩家、只把背包多余材料分类入库而保留食物、满箱后使用 NPC 背包中的箱子物品真实放置并继续整理，以及满箱且没有现成箱子/工作台时从原木递归制作工作台与箱子、真实放置后继续整理。脚本临时切换并最终恢复玩家原复活点，暂存并恢复 NPC 原背包与位置；所有测试物品带独立 NBT 标记，隔离家园与临时箱在 `finally` 清理，不访问玩家真实仓库。可用 `--scenario=retrieve|organize|expand|craft-expand` 单独运行固定场景。
...
- 2026-08-15 仓库递归制作扩容实机结果（Forge JAR SHA-256 `04093CFBF6DD93DAFE659C8DEBE55FF83546766FC9E2E40DCA5D18D3460741B8`）：隔离初始状态为已有仓库满载 `homeFiller=1728`，NPC 只有 3 个原木和 4 个待归档测试物品，木板、工作台与箱子均为 0。后台 `T` 触发同一个 `organize-storage` 任务后，NPC 递归制作木板、工作台和箱子，通过 FakePlayer/Forge 放置事件各真实放置 1 个工作台与 1 个箱子，并把 4 个物品存入新容器。最终 `homeSurplus=4`，NPC 中测试物品、原木、木板、工作台和箱子均为 0，`expanded=1,tables=1,tablePlacements=1,chestPlacements=1,direct=0,unknown=0`；流程返回 `localOnly=true`、`reversible=true`、`usedMinecraftTChat=true`，清理为 `storage-fixture:cleanup restored`。测试未使用物理键鼠、剪贴板或截图。
...
## 12. 2026-08-15 v0.1.6 仓库扩容与发布状态

- 满仓递归制作扩容的严格实机结果见第 4 节：后台 Minecraft `T` 触发 NPC 从 3 个原木开始制作并真实放置工作台和箱子，随后完成 4 个物品入库；工作台/箱子放置事件为 `1/1`，直接世界编辑和未知编辑为 `0/0`，现场最终返回 `storage-fixture:cleanup restored`。
- `v0.1.6` 最终回归为 workspace 273 项通过、1 项环境条件跳过，脚本验收 `169/169`，后台 HMCL/Minecraft 自动化测试和所有 workspace TypeScript 类型检查均通过；发布流水线中的 Forge 1.20.1 clean build 与完整 Forge 测试通过。
- 实机 Forge JAR SHA-256 为 `04093CFBF6DD93DAFE659C8DEBE55FF83546766FC9E2E40DCA5D18D3460741B8`；发布 clean build 的内嵌 Forge JAR SHA-256 为 `21689215749754A932E5FB24458379B7C5305330C0DB9ABB75E122DD4648E565`。两个归档均含 249 个文件，逐条目内容 SHA-256 差异为 0，整体哈希差异只来自 ZIP 容器元数据。
- `MinecraftCodexCompanion-Setup.exe` SHA-256 为 `1BACE95A6CB558F9755D579DDB784CB0DADF6A20704C1330CD06DF9EA4BE977B`；完整性为 `verified`，本地杀毒结果为 `clean`，签名状态为 `NotSigned`。隐私报告为 `localOnly=true`、`uploadedFiles=false`、`uploadedHashes=false`，且 Key、Base URL 配置、桥令牌、本地状态、世界存档和构建机路径六项包含检查均为 `false`。
- `MinecraftCodexCompanion-AgentKit-v0.1.6.zip` SHA-256 为 `8E85F7F831EC2848BBDF01ECE3B1C7F1DBCEA6AA6086384A3F1D7074F5741C64`，只含 9 个预期相对路径文件，隐私校验通过。它不包含游戏运行时、用户配置、Key、对话、日志或存档。

## 13. 2026-08-15 v0.1.6 食物生存重启修复与重新发布

- “给我16个肉”的严格实机验收已经通过，完整世界状态证据与隐私边界见第 4 节及 [`LIVE-FOOD-SURVIVAL-RESTART-ACCEPTANCE-20260815.json`](LIVE-FOOD-SURVIVAL-RESTART-ACCEPTANCE-20260815.json)。修复覆盖分批烹饪累计检查点、显式玩家食物任务接管自动补粮、按目标数量消耗燃料、冲突物品可逆恢复，以及隔离 HMCL 重启后继续连接同一个 loopback 控制服务。
- 当前回归为 workspace 273 项通过、1 项环境条件跳过，脚本验收 `170/170`，后台 HMCL/Minecraft 自动化测试和所有 workspace TypeScript 类型检查均通过。Forge 1.20.1 `clean build` 成功，进程内测试 `430/430`、0 失败、0 跳过。
- 重新发布内嵌 Forge JAR SHA-256 为 `EAC7663AF2B8F4BFB02F8B9189F0D7833D829A86BBA0D0BC68C0A54A3DB49593`。新的 `MinecraftCodexCompanion-Setup.exe` SHA-256 为 `2A121A0D1863ABCAED2E52D70B2F0C6A7F88F76F53404682323C5EFFA4D757A8`，载荷包含 5843 个文件；完整性为 `verified`，ClamAV 本地静态扫描为 `clean`，签名状态为 `NotSigned`。
- 发布隐私门禁确认 `localOnly=true`、`uploadedFiles=false`、`uploadedHashes=false`，且 API Key、Base URL 配置、桥令牌、本地状态、Minecraft 世界和构建机路径六项包含检查均为 `false`。AgentKit 仍为 9 个预期文件，SHA-256 为 `8E85F7F831EC2848BBDF01ECE3B1C7F1DBCEA6AA6086384A3F1D7074F5741C64`，隐私校验通过。
